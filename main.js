function getAllEvents() {
    fetch("http://birtondesign.com/mywpsite/wp-json/wp/v2/events?_embed")
        .then(res => res.json())
        .then(showEvents);
}

function getEventsByCategory(id) {
    fetch("http://birtondesign.com/mywpsite/wp-json/wp/v2/events?_embed&categories=" + id)
        .then(res => res.json())
        .then(showEvents);
}
...
function getSingleEventById(MyId) {
    fetch("http://birtondesign.com/mywpsite/wp-json/wp/v2/events/" + MyId + "/?_embed")
        .then(res => res.json())
        .then(showSingleEvent);
}

function getEventsByTag() {
    fetch("http://birtondesign.com/mywpsite/wp-json/wp/v2/categories")
        .then(res => res.json())
        .then(showTagMenu);
}
...
function showMenu(categories) {
    ...
    categories.forEach(function (categoryname) {
        if (window.location.href === "http://birtondesign.com/webapp/index.html?categoryid=" + categoryname.id + '#' || window.location.href === "http://birtondesign.com/webapp/index.html?categoryid=" + categoryname.id) {
...
function showTagMenu(categories) {
    ...
    categories.forEach(function (category) {
        if (window.location.href === "http://birtondesign.com/webapp/index.html?categoryid=" + category.parent + '#' || window.location.href === "http://birtondesign.com/webapp/index.html?categoryid=" + category.parent) {
